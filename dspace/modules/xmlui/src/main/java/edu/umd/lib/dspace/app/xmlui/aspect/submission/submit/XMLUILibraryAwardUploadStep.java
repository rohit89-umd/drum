 ...
 * user to upload files into the submission. The form is optimized for one file,
 * but allows the user to upload more if needed.
 * <P>
...
public class XMLUILibraryAwardUploadStep extends AbstractSubmissionStep
{
    ...
    /**
     * Global reference to edit file page (this is used when a user requests to
     * edit a bitstream)
     **/
    ...
    UIException, SQLException, IOException, AuthorizeException
    {
        ...
        // Part A:
        // First ask the user if they would like to upload a new file (may be
        // the first one)
        Division div = body.addInteractiveDivision("submit-upload", actionURL,
                Division.METHOD_MULTIPART, "primary submission");
        ...
        if (bitstreams.length > 0 || disableFileEditing)
        {
            ...
            if (!disableFileEditing)
            {
                ...
                actionRow.addCell();
                Button removeSeleceted = actionRow.addCell(null, null, 0, 6,
                        null).addButton("submit_remove_selected");
    ...
    AuthorizeException
    {
        ...
        for (Bitstream bitstream : bitstreams)
        {
            ...
            org.dspace.app.xmlui.wing.element.Item file = uploadSection
                    .addItem();
